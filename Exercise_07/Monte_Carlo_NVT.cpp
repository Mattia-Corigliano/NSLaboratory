...
{ 
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
      //Instant_values(istep, iblk); //comment if not necessary
...
//Print initial values for the potential energy and virial
  ofstream instant("instant_values.txt");
  instant << setw(15) << "#step" << setw(15) << "U/N" << setw(15) << "P" << endl << endl;
  instant << setw(15) << 0 << setw(15) << walker[iv]/(double)npart + vtail << setw(15) << rho * temp + (walker[iw] + (double)npart * ptail) / vol  << endl;
...
void Instant_values(int istep, int iblk){
  //saving instant values
  ofstream instant("instant_values.txt", ios::app);
  instant << setw(15) << istep+nstep*(iblk-1) << setw(15) << walker[iv]/(double)npart + vtail << setw(15) << rho * temp + (walker[iw] + (double)npart * ptail) / vol  << endl; 
  instant.close();
}
...
//g(r)
    //Gofr << "#" << setw(wd) << "block " << setw(wd) << "r" << setw(wd) << "g(r)" << endl << endl;
    //Gave << "#" << setw(wd) << "block " << setw(wd) << "r" << setw(wd) << "g(r)" << setw(wd) << "error" << endl << endl;
    for(int k=igofr;k<igofr+nbins;k++){
    	double r = k*bin_size;
    	double vol = 4.*pi*(pow(r+bin_size, 3.)-pow(r, 3.))/3.;
	gdir = blk_av[k]/(vol*rho*npart*blk_norm);
	glob_av[k] += gdir;
    	glob_av2[k] += gdir*gdir;
    	err_gdir=Error(glob_av[k],glob_av2[k],iblk);
	Gofr << " " << setw(wd) << iblk << setw(wd) << r << setw(wd) << gdir << endl;
	Gave << " " << setw(wd) << iblk << setw(wd) << r << setw(wd) << glob_av[k]/(double)iblk << setw(wd) << err_gdir << endl;
    }
